...
sealed class AstNodeBase {
    abstract suspend fun <T> accept(visitor: AstVisitor<T>): Any?
}

data class Program(val nodes: List<Statement>) : AstNodeBase() {
    override suspend fun <T> accept(visitor: AstVisitor<T>) = visitor.visitProgram(this)
}

sealed class Statement : AstNodeBase() {
    // this is needed because an override can't widen the input types
    final override suspend fun <T> accept(visitor: AstVisitor<T>) =
        accept(visitor as StatementVisitor<T>)

    abstract suspend fun <T> accept(visitor: StatementVisitor<T>): T
}

data class Print(val value: String) : Statement() {
    override suspend fun <T> accept(visitor: StatementVisitor<T>) = visitor.visitPrint(this)
}

data class Out(val expression: Expression) : Statement() {
    override suspend fun <T> accept(visitor: StatementVisitor<T>) = visitor.visitOut(this)
}

data class Var(val name: String, val expression: Expression) : Statement() {
    override suspend fun <T> accept(visitor: StatementVisitor<T>) = visitor.visitVar(this)
}

data class ExpressionStatement(val expression: Expression) : Statement() {
    override suspend fun <T> accept(visitor: StatementVisitor<T>) =
